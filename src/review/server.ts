...
import { finalizeEpisodeProduction } from "../series/finalize-service.js";
  ...
  const mainPromise = new Promise<void>((resolve, reject) => {
    ...
    const requestHandler = async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (url === "/api/series/finalize" && method === "POST") {
        try {
          const finalizeResult = await finalizeEpisodeProduction({
            seriesId,
            episodeNumber,
            script,
            bible,
            outputDir: options.outputDir,
            allowMockMedia: false, // Strict provenance: mock takes forbidden in official canon
          ...
          if (!finalizeResult.success) {
            ...
            res.end(
              JSON.stringify({
                success: false,
                error: finalizeResult.error || "Finalize thất bại.",
                missingApprovals: finalizeResult.missingApprovals || [],
                mockTakesDetected: finalizeResult.mockTakesDetected || [],
                qaReport: finalizeResult.qaReport || null,
              })
            );
            return;
          }
          ...
          res.end(
            JSON.stringify({
              ...
              commitId: finalizeResult.commitId || null,
              qaReport: finalizeResult.qaReport || null,
            })
          );
